...
#
#
# def compute_divided_difference(x0: Decimal, x1: Decimal, y0: Decimal, y1: Decimal) -> Decimal:
#     return (y0 * x1 - y1 * x0) / (x1 - x0)
#
#
# def get_lagrange_polynomials_values(x_data_points: list[Decimal], y_function_values: list[Decimal],
#                                     alpha: Decimal) -> Decimal:
#     initial_polynom_degree: int = len(x_data_points)
#     value: int = len(x_data_points) - 1
#     current_approximation: Decimal = Decimal(0)
#     initial_approximation: Decimal = interpolate_lagrange(x_data_points, y_function_values).subs('x', alpha)
#
#     while value > 0:
#         lagrange_polynomial = interpolate_lagrange(x_data_points[:-value], y_function_values[:-value])
...
#         value -= 1
#
#         print(f"Eroarea: {abs(current_approximation - initial_approximation)}")
...
#
# x_data_points: list[Decimal] = [Decimal('-1.432'), Decimal('-0.675'), Decimal('1.439'), Decimal('2.567'),
#                                 Decimal('3.486'),
#                                 Decimal('4.910'),
#                                 Decimal('5.763')]
#
# y_function_values: list[Decimal] = [Decimal('7.67103'), Decimal('5.45321'), Decimal('3.76129'), Decimal('0.56741'),
#                                     Decimal('-1.5630'),
#                                     Decimal('0.7684'),
#                                     Decimal('2.56793')]
#
